...
class CoreNet(nn.Module):
    def __init__(self, network_output_size=84, num_channels=4,type= None):
...
class DuelingHeadNet(nn.Module):
    def __init__(self, n_actions=4,type= None):
...
class EnsembleNet(nn.Module):
    def __init__(self, n_ensemble, n_actions, network_output_size, num_channels,type= None, dueling=False):
        super(EnsembleNet, self).__init__()
        self.core_net = CoreNet(network_output_size=network_output_size, num_channels=num_channels,type = type)
        self.dueling = dueling
        if self.dueling:
            print("using dueling dqn")
            self.net_list = nn.ModuleList([DuelingHeadNet(n_actions=n_actions,type = type) for k in range(n_ensemble)])
        else:
            self.net_list = nn.ModuleList([HeadNet(n_actions=n_actions,type = type) for k in range(n_ensemble)])
